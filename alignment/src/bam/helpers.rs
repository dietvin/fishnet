...
pub(super) fn unpack_tag<V>(get_tag_result: Result<V, BamReadError>, default_value: Option<V>) -> Result<Option<V>, BamReadError> {
...
pub(super) fn get_iarray_tag(bam_data: &Data, tag: &str) -> Result<Vec<i16>, BamReadError> {
...
/// * `Err(BamReadError)` - If the tag is missing, has an unexpected type or 
///                         the tag was found, but could not be extracted
pub(super) fn get_uint_tag(bam_data: &Data, tag: &str) -> Result<usize, BamReadError> {
    let tag_b: [u8; 2] = from_str(tag)?;
    match bam_data.get(&tag_b) {
...
/// * `Err(BamReadError)` - If the tag is missing, has an unexpected type or 
///                         the tag was found, but could not be extracted
pub(super) fn get_str_tag(bam_data: &Data, tag: &str) -> Result<String, BamReadError> {
    let tag_b: [u8; 2] = from_str(tag)?;
    match bam_data.get(&tag_b) {
...
/// * `Err(BamReadError)` - If the tag is missing, has an unexpected type or 
///                         the tag was found, but could not be extracted
pub(super) fn get_float_tag(bam_data: &Data, tag: &str) -> Result<f32, BamReadError> {
    let tag_b: [u8; 2] = from_str(tag)?;
    match bam_data.get(&tag_b) {
        // The tag was found and a float
...
pub(super) fn reverse_complement(seq: &Vec<u8>) -> Result<Vec<u8>, BamReadError> {
