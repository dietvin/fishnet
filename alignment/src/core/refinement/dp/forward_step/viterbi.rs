use crate::core::refinement::dp::forward_step::{
    RefinementAlgo, helpers::{LARGE_SCORE, score}
};
...
impl RefinementAlgo for Viterbi {
    #[inline(always)]
    fn forward_step(
        &self,
        scores: &mut [f32],
...
/// # Arguments
///
/// * `scores` - Mutable slice to be populated with forward Viterbi scores at each position
///              in the current base's band
/// * `traceback` - Mutable slice to be populated with traceback information; each value 
///                 indicates the number of signal points backward until the first point
...
pub(super) fn forward_step_viterbi(
    scores: &mut [f32],
    traceback: &mut [i32],
    previous_scores: &[f32],
    level: f32,
    ...
    if band_start_diff == 0 {
        ...
        scores[0] = LARGE_SCORE + previous_scores[previous_scores.len()-1];
        traceback[0] = -1;
    } else {
        // Compute move score for start of base band
        let base_score = score(level, signal[0]);
        scores[0] = previous_scores[band_start_diff-1] + base_score;
        traceback[0] = 0;
    }

    // Create slice of previous_scores starting at the same position as scores
    ...
    let process_len = if previous_scores_slice.len() == scores.len() {
    ...
    for band_pos in 1..=process_len {
        let base_score = score(level, signal[band_pos]);
        let move_score = previous_scores_slice[band_pos - 1] + base_score;
        let stay_score = scores[band_pos - 1] + base_score;

        if move_score < stay_score {
            scores[band_pos] = move_score;
            traceback[band_pos] = 0;
        } else {
            scores[band_pos] = stay_score;
            traceback[band_pos] = traceback[band_pos - 1] + 1;
        }
    }

    // Stay through rest of the band
    for band_pos in (process_len + 1)..scores.len() {
        let base_score = score(level, signal[band_pos]);
        let stay_score = scores[band_pos - 1] + base_score;
        scores[band_pos] = stay_score;
        traceback[band_pos] = traceback[band_pos - 1] + 1;
