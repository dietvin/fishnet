...
error is raised."
        ...
        .arg(
            Arg::new("output-batch-size")
                ...
                .long_help(
"Output batch size. Determines the estimated data size (in bytes) that
is collected in an output buffer before flushing and dumping to file.
Higher values reduce the I/O overhead, potentially increasing speed,
while requiring more memory."
        ...
        .arg(
            Arg::new("threads")
                ...
                .long_help(
"Set the number of parallel worker threads used during processing."
                )
        )
        .arg(
            Arg::new("queue-size")
                ...
                .long_help(
"Sets the queue size for transfering data to and from worker threads. 
Decrease queue size for a reduced memory footprint."
                )
        )
